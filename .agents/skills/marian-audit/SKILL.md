...
```bash
node .agents/skills/marian-audit/scripts/check-sources.mjs --file docs/khao-cuu/<id>/khao-cuu.json
```
...
```bash
node .agents/skills/marian-audit/scripts/check-image.mjs <đường/dẫn/ảnh...>
```
...
`references/ho-so-kiem-chung.md`, sao chép khung từ `.agents/skills/_lib/examples/kiem-chung.example.json`.
